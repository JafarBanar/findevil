...
This report is a living assessment of CaseTrace accuracy. The current final demo path uses a generated raw NTFS image because no Windows host is available.
...
- NTFS image status: Verified in `runs/realistic-windows-image` (2 iterations, 1 unsupported claim blocked, 0 tool failures)
...
- [x] List missed artifacts (controlled artifact-tree and generated NTFS image detected all 8 expected artifact categories; native/public disk evaluation pending).
